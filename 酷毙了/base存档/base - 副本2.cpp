...
LRESULT CALLBACK MouseOverlayWndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_PAINT: {
		...
		if (GetCursorInfo(&ci) && ci.flags == CURSOR_SHOWING) {
			...
			{ 
				adjustedX = pt.x - 2*offset.offsetX;
				adjustedY = pt.y - 2*offset.offsetY;
			}

			// 绘制鼠标
			DrawIconEx(hdcMem, adjustedX, adjustedY, g_currentCursor, 2* cursorWidth, 2*cursorHeight, 0, NULL, DI_NORMAL);
